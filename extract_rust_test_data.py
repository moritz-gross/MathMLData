...
def extract_from_files(file_list: List[str], expr_output: str, braille_output: str, canonical_output: str) -> None:
    ...
    print(f"\nTotal extracted: {total_expr_count} expr strings, {total_braille_count} braille strings, and {total_canonical_count} canonical MathML strings")
    print(f"Written to {expr_output}, {braille_output}, and {canonical_output}")


if __name__ == "__main__":
    if len(sys.argv) < 5:
        print("Usage: python extract_rust_test_data.py <expr_output_file> <braille_output_file> <canonical_output_file> <input_file1> [input_file2] ...")
        sys.exit(1)
